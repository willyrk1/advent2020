...
const answersToBits = answers => [...answers]
	.map(letter => 0x01 << (letter.charCodeAt(0) - aCode))
	.reduce(toUnion)
	
const countBits = bits => {
	let retVal = 0
	for (let mask = 0x01 << ('z'.charCodeAt(0) - aCode); mask; mask >>= 1)
		if (bits & mask)
			retVal++
	return retVal
}

const groupAnswersToCount = answersReducer => groupAnswers =>
	countBits(groupAnswers.split('\n').map(answersToBits).reduce(answersReducer))
	
const getAnswer = (allAnswers, answersReducer) =>
	allAnswers.map(groupAnswersToCount(answersReducer)).reduce(toSum)

const part1 = allAnswers => getAnswer(allAnswers, toUnion)
const part2 = allAnswers => getAnswer(allAnswers, toIntersection)

const outputFormatted = allAnswers =>
	`Day 6: Part 1 = ${part1(allAnswers)}, Part 2 = ${part2(allAnswers)}`
const output = allAnswersString => outputFormatted(allAnswersString
	.trim()
	.split('\n\n')
)
