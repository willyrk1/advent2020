...
const validHeights = [
	{ unit: 'cm', atLeast: 150, atMost: 193 },
	{ unit: 'in', atLeast: 59, atMost: 76 },
]

const heightCheck = value => ({ unit, atLeast, atMost }) => {
	if (new RegExp(`^\\d+${unit}$`).test(value)) {
		const num = value.replace(unit, '')
		return atLeast <= num && num <= atMost
	}
}

const validEyeColorLookup = { amb: 1, blu: 1, brn: 1, gry: 1, grn: 1, hzl: 1, oth: 1 }

const fourDigits = /^\d{4}$/

const requiredFields = [
	{ name: 'byr', isValid: value => fourDigits.test(value) && 1920 <= value && value <= 2002 },
	{ name: 'iyr', isValid: value => fourDigits.test(value) && 2010 <= value && value <= 2020 },
	{ name: 'eyr', isValid: value => fourDigits.test(value) && 2020 <= value && value <= 2030 },
	{ name: 'hgt', isValid: value => validHeights.some(heightCheck(value)) },
	{ name: 'hcl', isValid: value => /^#[0-9a-f]{6}$/.test(value) },
	{ name: 'ecl', isValid: value => validEyeColorLookup[value] },
	{ name: 'pid', isValid: value => /^\d{9}$/.test(value) },
]

const whereValid = checkPropertyValidity => fields => requiredFields.every(({ name, isValid }) =>
	fields[name] && (!checkPropertyValidity || isValid(fields[name]))
)

const fieldLookupReducer = (fieldLookup, line) => {
	const [key, value] = line.split(':')
	fieldLookup[key] = value;
	return fieldLookup;
}
const getFieldLookup = passport => passport.split(/\s/).reduce(fieldLookupReducer, {})
const part1 = passports => passports.filter(whereValid()).length
const part2 = passports => passports.filter(whereValid(true)).length

const outputFormatted = passports => `Day 4: Part 1 = ${part1(passports)}, Part 2 = ${part2(passports)}`
const output = passportsString => outputFormatted(passportsString.trim().split('\n\n').map(getFieldLookup))
