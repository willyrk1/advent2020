const getSeatCountByAdjacency = (seatLayout, row, col) => {
	let seatCount = 0
	if (row) {
		if (seatLayout[row - 1][col] == '#')
			seatCount++
		if (col && seatLayout[row - 1][col - 1] == '#')
			seatCount++
		if (col < seatLayout[row - 1].length - 1 && seatLayout[row - 1][col + 1] == '#')
			seatCount++
	}
	if (row < seatLayout.length - 1) {
		if (seatLayout[row + 1][col] == '#')
			seatCount++
		if (col && seatLayout[row + 1][col - 1] == '#')
			seatCount++
	...
	if (col && seatLayout[row][col - 1] == '#')
		seatCount++
	if (col < seatLayout[row].length - 1 && seatLayout[row][col + 1] == '#')
		seatCount++
	return seatCount
}

const getSeatCountBySightline = (seatLayout, row, col) => {
	let seatCount = 0
	let searchRow, searchCol
	for (searchRow = row - 1; searchRow >= 0; searchRow--)
		if (seatLayout[searchRow][col] == 'L') {
			break
		} else if (seatLayout[searchRow][col] == '#') {
			seatCount++
			break
		}
	for (searchRow = row + 1; searchRow < seatLayout.length; searchRow++)
		if (seatLayout[searchRow][col] == 'L') {
			break
		} else if (seatLayout[searchRow][col] == '#') {
			seatCount++
	...
	for (searchCol = col - 1; searchCol >= 0; searchCol--)
		if (seatLayout[row][searchCol] == 'L') {
			break
		} else if (seatLayout[row][searchCol] == '#') {
			seatCount++
			break
		}
	for (searchCol = col + 1; searchCol < seatLayout[row].length; searchCol++)
		if (seatLayout[row][searchCol] == 'L') {
			break
		} else if (seatLayout[row][searchCol] == '#') {
			seatCount++
	...
	for (
		searchRow = row - 1, searchCol = col - 1;
		searchRow >= 0 && searchCol >= 0;
		searchRow--, searchCol--
	)
		if (seatLayout[searchRow][searchCol] == 'L') {
			break
		} else if (seatLayout[searchRow][searchCol] == '#') {
			seatCount++
			break
		}
	for (
		searchRow = row + 1, searchCol = col - 1;
		searchRow < seatLayout.length && searchCol >= 0;
		searchRow++, searchCol--
	)
		if (seatLayout[searchRow][searchCol] == 'L') {
			break
	...
	for (
		searchRow = row - 1, searchCol = col + 1;
		searchRow >= 0 && searchCol < seatLayout[searchRow].length;
		searchRow--, searchCol++
	)
		if (seatLayout[searchRow][searchCol] == 'L') {
			break
	...
	for (
		searchRow = row + 1, searchCol = col + 1;
		searchRow < seatLayout.length && searchCol < seatLayout[searchRow].length;
		searchRow++, searchCol++
	)
		if (seatLayout[searchRow][searchCol] == 'L') {
			break
...
const runSeatingRules = (seatLayout, seatCounter, clearThreshold) => {
	let layoutCopy = seatLayout.map(seatingRow => seatingRow.split(''))
	while (true) {
		const newLayout = layoutCopy.map(row => [...row])
		let changeFound = false
		for (row = 0; row < layoutCopy.length; row++) {
			for (col = 0; col < layoutCopy[row].length; col++) {
				const seatCount = (seatLayout[row][col] !== '.')
					&& seatCounter(layoutCopy, row, col)
				switch (layoutCopy[row][col]) {
					case 'L':
						if (!seatCount) {
							newLayout[row][col] = '#'
							changeFound = true
...
const part1 = seatLayout => runSeatingRules(seatLayout, getSeatCountByAdjacency, 4)
const part2 = seatLayout => runSeatingRules(seatLayout, getSeatCountBySightline, 5)

const outputFormatted = seatLayout =>
	`Day 11: Part 1 = ${part1(seatLayout)}, Part 2 = ${part2(seatLayout)}`
	
const output = seatLayoutString => outputFormatted(seatLayoutString
	.trim()
	.split('\n')
)
