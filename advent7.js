const toCount = rules => (totalCount, [bag, count]) =>
	totalCount
	+ count
	+ (rules[bag] ? count * Object.entries(rules[bag]).reduce(toCount(rules), 0) : 0)

const whereContains = (rules, target) => bags =>
	bags[target]
	|| Object.keys(bags).some(bag => rules[bag] && whereContains(rules, target)(rules[bag]))

const part1 = rules => Object.values(rules).filter(whereContains(rules, 'shiny gold')).length
const part2 = rules => Object.entries(rules['shiny gold']).reduce(toCount(rules), 0)

const toQuantityObject = (contents, bag) => {
	const [, quantityString, bagString] = bag.match(/(\d*) (.*)/)
	contents[bagString] = +quantityString
	return contents
}

const toContainTree = (tree, [container, containsList]) => {
	tree[container] = containsList.reduce(toQuantityObject, {})
	return tree
}

const outputFormatted = rules =>
	`Day 7: Part 1 = ${part1(rules)}, Part 2 = ${part2(rules)}`
const output = rulesString => outputFormatted(rulesString
	.trim()
	.split('\n')
	.map(rule => rule.replace(/(?:\.| bags?)/g, '').split(' contain '))
	.map(([container, containsList]) => [container, containsList.split(', ')])
	.reduce(toContainTree, {})
)
