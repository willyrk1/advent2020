...
const doMath = expression => expression.split(' ').reduce((acc, curr) => {
	if (acc.isNumber) {
		if (acc.operand === '+')
			acc.value += +curr
		else
	...
	acc.isNumber = !acc.isNumber
	return acc
}, { isNumber: true, value: 0, operand: '+' }).value

const part1 = expressions => expressions.map(expression => {
	const parensRegex = /\(([^()]*)\)/
	let parenMatch = expression.match(parensRegex)
	while (parenMatch) {
		const value = doMath(parenMatch[1])
		expression = expression.replace(parenMatch[0], value)
		parenMatch = expression.match(parensRegex)
	}
	return doMath(expression)
}).reduce((x, y) => x + y)

const part2 = expressions => expressions.map(expression => {
	const parensRegex = /\(([^()]*)\)/
	const additionsRegex = /\d+( \+ \d+)+/g
	while (true) {
		let parenMatch = expression.match(parensRegex)
		while (parenMatch) {
			const [fullMatch, subExpression] = parenMatch
			const value = (subExpression.includes('+') && subExpression.includes('*'))
				? fullMatch.replace(additionsRegex, '($&)')
				: doMath(parenMatch[1])
			expression = expression.replace(fullMatch, value)
			parenMatch = expression.match(parensRegex)
		}
		if (expression.includes('+') && expression.includes('*'))
			expression = expression.replace(additionsRegex, '($&)')
		else
			return doMath(expression)
	}
}).reduce((x, y) => x + y)

const outputFormatted = expressions =>
	`Day 18: Part 1 = ${part1(expressions)}, Part 2 = ${part2(expressions)}`
	
const output = expressionsString => outputFormatted(expressionsString
	.trim()
	.split('\n')
)
